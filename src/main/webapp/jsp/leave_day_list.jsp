<%@page language="java" pageEncoding="utf-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title>请假管理</title>
<%
	String path = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
			+ request.getContextPath();
%>
<link href="<%=path%>/css/bootstrap/bootstrap.css" rel="stylesheet"
	type="text/css">
<link href="<%=path%>/css/bootstrap/bootstrap-theme.css" rel="stylesheet"
	type="text/css">
<link href="<%=path%>/css/styles.css" rel="stylesheet" type="text/css">
<link href="<%=path%>/css/labourers_family.css" rel="stylesheet"
	type="text/css">
</head>
<body>
	<div class="container-fluid">
		<%@include file="base/header.jsp"%>
		<div class="content-div row">
			<%@include file="base/left.jsp"%>
			<br>
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3"></div>
			<div
				class="col-lg-8 col-md-8 col-lg-8 col-xs-8 text-center form-back">
				<ul class="nav nav-tabs">
					<li class="now-active"><a href="<%=path%>/leaveDayList.do">记录查询</a></li>
					<li ><a href="<%=path%>/createLeaveDayPage.do">创建记录</a></li>
				</ul>
				<br>
				<div class="row text-primary">
					<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2"></div>
					<div class="col-lg-8 col-md-8 col-sm-8 col-xs-8">
						<!-- 查询条件表单 -->
						<form action="/cheer/queryLeaveDay.do" method="post" id="query_param">
							<div class="form-group row">
								<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
									<!-- 查询页号 -->
									<input type="hidden" name="queryPage" id="query_page" value="1">
									<!-- 员工号 -->
									<input type="text" name="empno" value="${empno}" class="form-control"
										placeholder="输入员工号查询">
								</div>
								<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
									<!-- 起始时间 -->
									<input type="date" pattern="yyyy-MM-dd" name="startTime" value="${startTime}" class="form-control"
										placeholder="输入开始时间号">
								</div>
								<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
									<!-- 结束时间 -->
									<input type="date" pattern="yyyy-MM-dd" name="endTime" value="${endTime}" class="form-control"
										placeholder="输入结束时间号">
								</div>
								<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
									<button class="btn btn-success" type="submit">查询</button>
								</div>
							</div>
						</form>
					</div>

				</div>
				<!-- 显示部分 -->
				<div class="row">
					<div>
						<!-- 显示请假信息 -->
						<table class="table table-striped table-bordered table-reponsive">
							<tr>
								<th class="text-center">工号</th>
								<th class="text-center">请假日期</th>
								<th class="text-center">请假天数</th>
								<th class="text-center">操作</th>
							</tr>
							<c:choose>
								<c:when test="${not empty iLeaveDayList}">
									<c:forEach items="${iLeaveDayList}" var="iLeaveDay">
										<tr>
											<td class="text-center">${iLeaveDay.empno}</td>
											<td class="text-center">${iLeaveDay.leaveDateAsStr}</td>
											<td class="text-center">${iLeaveDay.days}</td>
											<td class="text-center">
												<button class="update-button btn btn-primary btn-sm"
													data-path="<%=request.getContextPath() %>"
													data-staff-id="${iLeaveDay.id}">修改</button>
											</td>
										</tr>
									</c:forEach>
									<tr>
										<td colspan="4" class="text-center">
											<label>期间共请假${totalDays}天</label>
										</td>
									</tr>
								</c:when>
							</c:choose>
						</table>
					</div>
				</div>
				<!-- 分页标签 -->
				<div class="row text-primary">
					<div>
						<input type="hidden" id="current_page" value="${current_page }" />
						<input type="hidden" id="max_page" value="${max_page }" />
						<c:if test="${not empty current_page }">
							<ul class="pagination">
								<li><a href="#" class="page_tag" data-current="1"><i
										class="glyphicon glyphicon-fast-backward"></i></a></li>
								<li><a href="#" class="page_tag"
									data-current="${current_page-1}" ><i
										class="glyphicon glyphicon-backward"></i></a></li>
								<c:forEach begin="1" end="${max_page}" varStatus="status">
									<li class=""><a href="#" class="page_tag num_tag"
										data-current="${status.count}">${status.count}</a></li>
								</c:forEach>
								<li><a href="#" class="page_tag"
									data-current="${current_page+1}"><i
										class="glyphicon glyphicon-forward"></i></a></li>
								<li><a href="#" class="page_tag" data-current="${max_page}">
										<i class="glyphicon glyphicon-fast-forward"></i>
								</a></li>
							</ul>
						</c:if>
						<c:if test="${not empty leave_day_msg }">
							<div class="text-danger h4">${leave_day_msg }</div>
						</c:if>
					</div>
					<div><label class="text-danger h4">${leave_day_list_msg}</label></div>
				</div>
			</div>
		</div>
	</div>

	<script src="<%=path%>/js/bootstrap/jquery-1.12.js"></script>
	<script src="<%=path%>/js/bootstrap/bootstrap.js"></script>
	<script src="<%=path%>/js/leaveDay.js"></script>
</body>
</html>
